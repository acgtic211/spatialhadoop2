 ...
 * @author eldawy
 */
public class KNNJVoronoi {

    /**
     * Class logger
     */
    private static final Log LOG = LogFactory.getLog(KNNJVoronoi.class);
            ...
            if (outputValue.index == 0) {
                ...
                if (!cellMbr.isValid() || cellMbr.contains(shape_mbr.x1, shape_mbr.y1)) {
                    ...
                    if(newSize>0){
                        ...
                        VoronoiKMeansPartitioner qp = qtrees.get((long)Math.floor(((double)auxCellId)/((double)SUBCELL_ID))+"");
    ...
    public static <S extends Shape> long KNNJ(Path[] inFiles, Path userOutputPath, OperationsParams params)
        ...
        JobConf job = new JobConf(params, KNNJVoronoi.class);
