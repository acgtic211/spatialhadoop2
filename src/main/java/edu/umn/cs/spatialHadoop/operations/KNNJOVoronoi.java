...
import edu.umn.cs.spatialHadoop.indexing.VoronoiOpticsPartitioner;
 ...
 * @author eldawy
 */
public class KNNJOVoronoi {

    /**
     * Class logger
     */
    private static final Log LOG = LogFactory.getLog(KNNJOVoronoi.class);
    ...
    public static class KNNJReduce1<S extends Shape> extends MapReduceBase
        implements Reducer<Partition, IndexedText, IntWritable, VoronoiOpticsPartitioner> {
        /**
         * Class logger
         */
        ...
        public void reduce(Partition cellId, Iterator<IndexedText> values,
                           final OutputCollector<IntWritable, VoronoiOpticsPartitioner> output, Reporter reporter) throws IOException {
            ...
            VoronoiOpticsPartitioner qt = new VoronoiOpticsPartitioner();
    ...
    public static class KNNJMap2 extends MapReduceBase implements Mapper<Rectangle, Text, LongWritable, IndexedText> {
        ...
        private Path[] inputFiles;
        private InputSplit currentSplit;
        private Map<String, VoronoiOpticsPartitioner> qtrees;
        ...
        public void configure(JobConf job) {
            ...
            if(sizesAux!=null){
                ...
                qtrees = new HashMap<String, VoronoiOpticsPartitioner>();
                for(String key : aux.keySet()) {
                    VoronoiOpticsPartitioner qp = new VoronoiOpticsPartitioner();
        ...
        public void map(Rectangle cellMbr, Text value, final OutputCollector<LongWritable, IndexedText> output,
            ...
            if (!cellMbr.isValid() || cellMbr.contains(shape_mbr.x1, shape_mbr.y1)) {
                ...
                for (final Partition part : cells) {
                    ...
                    VoronoiOpticsPartitioner qp = qtrees.get(part.cellId+"");
                    if(shape instanceof Point){
                        qp.setReadOnly(true);
    ...
    public static class KNNJReduce2<S extends Shape> extends MapReduceBase
        ...
        public void reduce(LongWritable cellId, Iterator<IndexedText> values,
            ...
            VoronoiOpticsPartitioner qp = qtrees.get(parent+"");
            ...
            if (outputValue.index == 0) {
                ...
                if (!cellMbr.isValid() || cellMbr.contains(shape_mbr.x1, shape_mbr.y1)) {
                    ...
                    if(newSize>0){
                        ...
                        VoronoiOpticsPartitioner qp = qtrees.get((long)Math.floor(((double)auxCellId)/((double)SUBCELL_ID))+"");
    ...
    public static <S extends Shape> long KNNJ(Path[] inFiles, Path userOutputPath, OperationsParams params)
        ...
        JobConf job = new JobConf(params, KNNJOVoronoi.class);
