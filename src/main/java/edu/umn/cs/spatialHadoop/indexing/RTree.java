...
public class RTree<T extends Shape> implements Writable, Iterable<T>, Closeable {
  ...
  public static void spatialBinaryClosestPair(final RTree<Shape> R, final RTree<Shape> S, final int numberk, final Double delta,
                                              final ResultCollector2<NullWritable, DistanceAndPair> resultCollector2, final Reporter reporter) {
    try {
      final NullWritable Dummy = NullWritable.get();

      org.apache.hadoop.util.PriorityQueue<DistanceAndPair> ho;

      if (R.treeStartOffset >= 0 && S.treeStartOffset >= 0) {
        // Both trees are read from disk
        LOG.info("DISCOOOOO");
        ho = spatialBinaryClosestPairDisk(R, S, numberk, delta, resultCollector2, reporter);
      } else {
        LOG.info("MEMORIA");
        ho = spatialBinaryClosestPairMemory(R, S, numberk, delta, resultCollector2, reporter);
      }

      if (ho == null) {
        LOG.error("Error null closest pair for input of size: " + R.treeSize + " " + S.treeSize);
      }

      LOG.info("Found the closest pair: " + ho.size());

      DistanceAndPair t = new DistanceAndPair();
      while (ho.size() > 0) {
        DistanceAndPair aux = ((DistanceAndPair) ho.pop());
        t.distance = aux.distance;
        t.pair.first = aux.pair.first.clone();
        t.pair.second = aux.pair.second.clone();
        resultCollector2.collect(Dummy, t);
      }
    } catch (TopologyException e) {
      e.printStackTrace();
      return;
  ...
  protected static <S1 extends Shape, S2 extends Shape> org.apache.hadoop.util.PriorityQueue<DistanceAndPair> spatialBinaryClosestPairMemory(final RTree<S1> R,
                                                                                                                                             final RTree<S2> S, final int numberk, final Double delta, final ResultCollector2<NullWritable, DistanceAndPair> output, final Reporter reporter)
      throws IOException {
    ...
    return SpatialAlgorithms.SpatialBinaryClosestPair_planeSweep(rs, ss, numberk, delta, null, reporter);

  }

  protected static <S1 extends Shape, S2 extends Shape> org.apache.hadoop.util.PriorityQueue<DistanceAndPair> spatialBinaryClosestPairDisk(final RTree<S1> R,
                                                                                                                                           final RTree<S2> S, final int numberk, final Double delta, final ResultCollector2<NullWritable, DistanceAndPair> output, final Reporter reporter)
      throws IOException {

    Double gdmax = delta;

    class TreePairs implements Comparable{

      long nodes;
      double mindistance;

      public TreePairs(long nodes, double mindistance){
        this.nodes = nodes;
      ...
      public int compareTo(Object o) {
        ...
        if(mindistance<((TreePairs)o).mindistance)
          return -1;
    ...
    PriorityQueue<TreePairs> nodesToJoin = new PriorityQueue<TreePairs>(R.nodeCount + S.nodeCount);

    // Start with the two roots
    nodesToJoin.add(new TreePairs(0L,-1));
    ...
    while (!nodesToJoin.isEmpty()) {
      TreePairs tp = nodesToJoin.poll();
      long nodes_to_join = tp.nodes;
      int r_node = (int) (nodes_to_join >>> 32);
      int s_node = (int) (nodes_to_join & 0xFFFFFFFF);
      LOG.info("COMPARAMOS "+r_node +":"+s_node);
      if (gdmax!=-1){
        if(tp.mindistance > gdmax) {
          LOG.info("SALTAMOS "+tp.mindistance+":"+r_node +":"+s_node);
          continue;
        }else if(full && tp.mindistance == gdmax){
          LOG.info("SALTAMOS "+tp.mindistance+":"+r_node +":"+s_node);
          continue;
        }
      }
      ...
      if (!r_leaf && !s_leaf) {
        ...
        for (int i = 0; i < R.degree; i++) {
          ...
          for (int j = 0; j < S.degree; j++) {
            int new_s_node = s_node * S.degree + j + 1;
            double distance = R.nodes[new_r_node].getMinDistance(S.nodes[new_s_node]);
            if (gdmax==-1|| distance <= gdmax) {
              if(full && distance == gdmax){
                continue;
              }
              long new_pair = (((long) new_r_node) << 32) | new_s_node;
              nodesToJoin.add(new TreePairs(new_pair,distance));
            }
          }
        }
        ...
        for (int j = 0; j < S.degree; j++) {
          int new_s_node = s_node * S.degree + j + 1;
          double distance = R.nodes[r_node].getMinDistance(S.nodes[new_s_node]);
          if (gdmax==-1||distance <= gdmax) {
            if(full && distance == gdmax){
              continue;
            }
        ...
        for (int i = 0; i < R.degree; i++) {
          int new_r_node = r_node * R.degree + i + 1;
          double distance = R.nodes[new_r_node].getMinDistance(S.nodes[s_node]);
          if (gdmax==-1||distance <= gdmax) {
            if(full && distance == gdmax){
              continue;
            }
        ...
        LOG.info("CALCULANDO SBCP");
        org.apache.hadoop.util.PriorityQueue<DistanceAndPair> ho = SpatialAlgorithms.SpatialBinaryClosestPair_planeSweepClassic(r_records, s_records, numberk, gdmax==-1?null:gdmax, kcpho, reporter);
        if(ho!=null&&ho.size()==numberk){
          gdmax = ho.top().distance;
        }
        LOG.info("DELTA "+gdmax);
        result_count = ho.size();
        full = result_count == numberk;
